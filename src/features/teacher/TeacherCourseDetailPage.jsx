...
export default function TeacherCourseDetailPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="bg-card rounded-xl shadow-sm p-6 flex items-start justify-between">
           ...
           <Tabs defaultValue="assignments">
               ...
               <TabsContent value="students" className="mt-6 space-y-3">
                  ...
                  {displayedStudents.map((s) => (
                    <div key={s.id} className="p-3 border border-border rounded-lg bg-card">
                      <p className="font-medium text-foreground">{s.name}</p>
                      <p className="text-sm text-muted-foreground">{s.email}</p>
                      {s.is_mock && <span className="text-xs text-muted-foreground">(Mock Data)</span>}
                    </div>
                  ))}
               ...
               <TabsContent value="announcements" className="mt-6 space-y-3">
                  {mockAnnouncements.map((a) => (
                    <div key={a.id} className="p-3 border border-border rounded-lg bg-card">
